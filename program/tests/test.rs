...
async fn setup() -> (BanksClient, Keypair, Hash) {
    let mut program_test = ProgramTest::new(
        ...
        "my_project_program",
        my_project_api::ID,
        processor!(my_project_program::process_instruction),
    );
...
async fn run_test() {
    ...
    assert_eq!(counter_account.owner, my_project_api::ID);
    ...
    let balance = banks.get_balance(payer.pubkey()).await.unwrap();

    // Submit add transaction.
    let ix = transfer_sol(payer.pubkey(), Pubkey::new_unique(), balance - 10000000);
    let tx = Transaction::new_signed_with_payer(&[ix], Some(&payer.pubkey()), &[&payer], blockhash);
    let res = banks.process_transaction(tx).await;
    assert!(res.is_ok());
}
